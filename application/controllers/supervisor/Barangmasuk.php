...
class Barangmasuk extends CI_Controller {
	...
	#	{
        ...
        $data['Barang']=$this->db->query("SELECT barang_masuk.id_brgmasuk, barang_masuk.total, barang_masuk.tanggal, barang_masuk.part_no, suplier.nama_suplier,barang.nama_barang from barang_masuk join suplier on barang_masuk.id_suplier=suplier.id_suplier JOIN barang on barang_masuk.part_no=barang.part_no where nama_barang like '%".$data['search']."%'")->result();
   ...
   $data['supplier']=$this->db->query("select * from suplier")->result();
		$this->load->view('barang/v_data_barang_masuk_supervisor',$data);
   # }
  #else {
...
//        $queryambil=$this->db->query("select total from barang where part_no='$part_no'");
    ...
    if (!empty($part_no))
         {
        $this->m_crud->insert_data($data,'barang_masuk');
    ...
    function hapus($id){
        ...
        $this->m_crud->hapus_data($where,'barang_masuk');
        $this->session->set_flashdata('msg', 
                    '<div class="alert alert-success">
